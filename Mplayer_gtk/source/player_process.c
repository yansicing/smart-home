...
int player_start(PLAYER *player)
{
	...
	send_cmd_to_player(player, CMD_VOLUME, &player->status.volume);//首先暂停
...
/*
void player_lrc_init(PLAYER *player)
{
	LRC *temp = NULL;
	temp = dispose_lrc("../lyrics/coffee.lrc", &player->list.lrc);
	if(temp == NULL)
		return;
	int i;
	char buf[2000] = "";
	for(i=0;i<player->list.lrc.lrc_arry_size;i++)
	{
		char temp_buf[200] = "";
		gb2312_to_utf8((unsigned char *)player->list.lrc.lrc_arry[i]->lrc, (unsigned char *)temp_buf);
		strcat(buf, temp_buf);
		strcat(buf, "\n");
	}
	//gtk_label_set_text(GTK_LABEL(player->win.lrc_light), buf);
}*/
