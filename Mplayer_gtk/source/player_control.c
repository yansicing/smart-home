...
static int romote_control_mplayer(PLAYER *player, char *str)
{
	int cmd;
	sscanf(str, "ROMOTE_CMD=%d", &cmd);
	switch(cmd){
		case 0:
			player_auto_play(player, CMD_PAUSE);
			break;
		case 1:
			player_auto_play(player, CMD_FRONT);
			break;
		case 2:
			if(player->status.volume < 100)
				player->status.volume += 10;
			player_auto_play(player, CMD_VOLUME);
			break;
		case 3:
			if(player->status.volume > 0)
				player->status.volume -= 10;
			player_auto_play(player, CMD_VOLUME);
			break;
		default:
...
void mplayer_exit(PLAYER *player)
{
...
static int rcv_player_msg(PLAYER *player)
{
	...
	char *msg_cmp[] = {"ANS_TIME_POSITION","ANS_PERCENT_POSITION",
						"ANS_LENGTH","ANS_META_ARTIST",
						"ROMOTE_CMD", "MPLAYER_EXIT"}; //"ROMOTE_NEXT", "ROMOTE_VOL_ADD","ROMOTE_VOL_SUB"
	...
	for(i=0;i<sizeof(msg_cmp)/sizeof(msg_cmp[0]);++i)
	{
		...
		if(position != NULL)
		{
			switch(i)
			{
				...
				case 4:
					//printf("buf-=====%s====\n", buf);
					romote_control_mplayer(player, buf);
					break;
...
void *pthread_rcv_player_msg(void *arg)
{
	...
	while(1)
	{
		...
		if(player->status.percent>=99 && restart_flag==0){
			restart_flag = 1;
			player_auto_play(player, CMD_FRONT);/*下一曲*/
		}
